<template>
  ...
  <div class="geral w3-container extender-div-tela-toda w3-small" style="padding:0px;">
    ...
    <div class="margin-top-80 padding-10">
      ...
      <the-mask :mask="['#####-###']"  id="cep" class="w3-input w3-text-black bold-500 margin-top-30" type="text" v-model="cep" placeholder="Cep:"/>
      <input  id="rua" class="w3-input w3-margin-top w3-text-black bold-500" placeholder="Logradouro:" v-model="street" />
      <input id="numero" class="w3-input w3-margin-top w3-text-black bold-500" v-model="residenceNumber" type="text" placeholder="Numero:"/>
      <input id="complemento" class="w3-input w3-margin-top w3-text-black bold-500" v-model="complement" type="text" placeholder="Complemento:"/>
      <input id="bairro" class="w3-input w3-margin-top w3-text-black bold-500" v-model="district" type="text" placeholder="Bairro:"/>
      
     
      <select 
        class="w3-select w3-margin-top w3-text-black bold-500" 
        style="max-width: 100%!important" 
        id="estado" name="option"
        v-model="state"
      >
        <option value="" class="w3-col">Estado</option>
        <option class="w3-col" v-for="estado in estados">{{estado.nome}}</option>
      </select>
      <input id="cidade" class="w3-input w3-margin-top w3-text-black bold-500" v-model="city" type="text" placeholder="cidade:"/>
    </div>
    ...
    export default {
      ...
      methods: {
        ...
        async prencherTela(){
          let dados_estados
          try {
            dados_estados = await api.get('http://servicodados.ibge.gov.br/api/v1/localidades/estados')
            this.estados = dados_estados.data
          } catch (error) {
            this.$refs.enviarMensagem.exclamar("erro", "Não foi possivel buscar estado")
          }

          let complementar
          try {
            complementar = await api.get("/infoAdress/"+ this.dados_pessoais.id_usuario)
          } catch (error) {
            this.$refs.enviarMensagem.exclamar("erro", "Não existe dados a mostrar, por favor complete seus dados")
          }
        ...
        async atualizar(){
          let sucesso = false
          await api.put("/updateAdress/"+ this.dados_pessoais.id_usuario, {
            cep: this.cep,
            street: this.street,
